...
RES_DIR = os.path.expanduser(
    '~/weather-interpretability/results/patching_19var'
...
for fp in files:
    ...
    try:
        ...
        if len(d.files) != EXPECTED_ARRAYS:
            skipped.append((label, f'{len(d.files)} arrays')); continue
        ...
        for lead in LEADS:
            ...
            for out in NAMES:
                ...
                acc_base = acc(base, tr, cl)
                sd = float(np.std(tr))
                for pat in NAMES:
                    p = d[f'patch{pat}_lead{lead}_{out}']
                    rows.append({
                        'date': label, 'lead': lead, 'patched': pat, 'output': out,
                        'rel_sens': rmse(p, base) / sd if sd > 1e-9 else np.nan,
                        'dacc': acc(p, tr, cl) - acc_base,
                    })
        del d
        print(f'  {label} ok', flush=True)
    except Exception as e:
        skipped.append((label, str(e)[:40]))
...
for col, lead in enumerate(LEADS):
    sub = df[df.lead == lead]
    m1 = sub.groupby(['patched', 'output'])['rel_sens'].mean().unstack().reindex(
        index=NAMES, columns=NAMES).values
    m2 = sub.groupby(['patched', 'output'])['dacc'].mean().unstack().reindex(
        index=NAMES, columns=NAMES).values
    im1 = draw(axes[0, col], m1,
               f'Relative sensitivity  RMSE(patched,baseline)/std(truth)  --  {MODEL.capitalize()} +{lead}h   (n={n} dates)',
               'viridis', False)
    plt.colorbar(im1, ax=axes[0, col], shrink=0.8)
    im2 = draw(axes[1, col], m2,
               f'$\\Delta$ACC vs baseline  --  {MODEL.capitalize()} +{lead}h   (n={n} dates)', 'RdBu_r', True)
    plt.colorbar(im2, ax=axes[1, col], shrink=0.8)

plt.tight_layout()
out = f'{FIG_DIR}/matrix_19var_{MODEL}_n{n}.png'
