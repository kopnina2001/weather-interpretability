...
N_STEPS = 4
K_LANDMARKS = 256  # matches the k chosen for the Bebinca case via rank-vs-error sweep
OUT_PATH = os.path.expanduser('~/weather-interpretability/results/legacy/skeleton_decomposition/quiet_aurora.pkl')
...
raw_X = {}  # cache the raw (H*W, D) matrices so a k-sweep doesn't need to rerun the model
for stage_idx, tensors in captured.items():
    ...
    for step_idx, x in enumerate(tensors):
        ...
        raw_X[f'stage{stage_idx}_step{step_idx}'] = X.astype(np.float32)
...
RAW_X_PATH = os.path.expanduser('~/weather-interpretability/results/legacy/skeleton_decomposition/quiet_aurora_raw_X.npz')
np.savez_compressed(RAW_X_PATH, **raw_X)
print(f'Saved raw activation matrices to {RAW_X_PATH}', flush=True)
