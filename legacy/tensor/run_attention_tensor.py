"""Task 3: capture the full attention WEIGHT tensor (heads, query, key) = (heads, 144, 144)
for the same window (Bebinca landfall area), block 0 of stages 0/1, both dates. Unlike task 2
(CKA on per-head output representations), here we decompose the tensor of attention matrices
itself with Tucker decomposition, to ask: how many "modes" along the head axis are needed to
reconstruct all heads' attention patterns? A low required head-rank means heads are largely
redundant combinations of a few shared attention patterns; a high required rank means each
head routes information in a genuinely distinct way.
"""
import os
import sys
import glob
...
for date_label, init_time in DATES.items():
    ...
    for stage_idx in STAGES_TO_ANALYZE:
        ...
        block0.attn.forward = types.MethodType(make_eager_forward(stage_idx), block0.attn)
    ...
    for stage_idx in STAGES_TO_ANALYZE:
        ...
        for step_idx, attn in enumerate(captured_attn[stage_idx]):
            arr = attn[window_flat_idx].numpy()  # (heads, 144, 144)
            save_dict[f'stage{stage_idx}_step{step_idx}'] = arr

    out_path = os.path.join(OUT_DIR, f'{date_label}_attn_tensor.npz')
