...
RESULTS_DIR = os.path.expanduser('~/weather-interpretability/results/campaign/campaign_stormer')
...
for idx, init_time in enumerate(init_dates):
    ...
    clim_surf = climatology_regridded[month]['surface']
    clim_upp = climatology_regridded[month]['upper']
    ...
    with torch.no_grad():
        for step in range(1, N_STEPS + 1):
            ...
            for i, v in enumerate(SURFACE_MAP):
                ...
                step_acc[v] = acc(pred_physical[i], truth[i], clim_surf[i])
            ...
            for vi, v in enumerate(UPPER_VARS_LONG):
                for li, lvl in enumerate(PRESSURE_LEVELS_HPA):
                    ...
                    step_acc[key] = acc(pred_physical[ch], truth[ch], clim_upp[vi, li])
