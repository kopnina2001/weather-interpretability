"""Compute all metrics for the input-patching experiment (results/legacy/patching_experiment/*.npz)
and save a tidy summary table + the raw fields needed for error maps, so the notebook only
needs to load small cached arrays instead of the ~6.7GB npz files.
"""
import os
import pickle
import numpy as np
...
lat_full = ds_surf.latitude.values[:720]  # Aurora crops to 720 latitudes (drops south pole row)
...
def storm_weight(valid_time, lat, lon):
    ...
    mslp = ds_surf.sel(time=valid_time)['mean_sea_level_pressure'].values[:720]
    sub = mslp[region_idx]
