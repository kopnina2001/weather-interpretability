"""Input-level climatological patching experiment for Aurora, redesigned per the "diagonal
7x7 matrix" plan: patch ONE atomic named variable at a time (not grouped categories), all at
levels 500/850hPa or surface -- which happen to be exactly the 3 levels (500,850,1000) our WB2
climatology covers, so there is NO partial-column caveat here (unlike the earlier storm-case
experiments): every patch is a full, unrestricted climatological replacement.

48 init dates (2024-2025, day 8 and 23 of each month, init hour cycling 00/06/12/18 evenly),
global scope only. This script processes a CHUNK of those 48 dates (pass chunk index 0-3 as
argv[1] when running 4 parallel processes on 4 GPUs, 12 dates each).

Only the 7 core variables (not the full 13-level x 5-var state) are kept for the 2 lead times
we care about (+6h, +24h) -- trimmed before saving, to keep output files small and I/O fast.
"""
import os
import sys
import glob
...
OUT_DIR = os.path.expanduser('~/weather-interpretability/results/patching_experiment_multiseason')
os.makedirs(OUT_DIR, exist_ok=True)
N_STEPS = 4  # need all 4 to reach +24h autoregressively, but only keep steps 0 (+6h) and 3 (+24h)
KEEP_STEP_IDX = [0, 3]
LEAD_LABELS = {0: 6, 3: 24}

# ---- build the 48 init dates: day 8 & 23 of every month, 2024-2025, hour cycling 0/6/12/18 ----
...
date_labels_sorted = list(ALL_DATES.keys())
chunk_labels = date_labels_sorted[CHUNK_IDX::N_CHUNKS]  # interleaved split: spreads chunks across the whole year range
DATES = {k: ALL_DATES[k] for k in chunk_labels}
print(f'Chunk {CHUNK_IDX}/{N_CHUNKS}: {len(DATES)} dates: {list(DATES.keys())}', flush=True)
...
# component name -> (kind, variable, level or None) -- these are also exactly the 7 output
# variables we measure, giving the square/diagonal matrix structure.
COMPONENTS = {
...
def get_climatology(time_val):
    ...
    # upper_clim keyed by (var, level) for direct lookup, since we only ever patch single levels
...
def run_rollout(surf0, upper0, surf1, upper1, init_time):
    ...
    batch = batch.to('cuda')
    kept = {}  # lead_hours -> dict of {core_var_name: 2D array}
    ...
    return kept  # {6: {...7 fields...}, 24: {...7 fields...}}
...
print(f'Chunk {CHUNK_IDX} all done.', flush=True)
